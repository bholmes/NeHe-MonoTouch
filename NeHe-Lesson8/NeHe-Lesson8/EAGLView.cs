...
namespace NeHeLesson8
{
	...
	public class EAGLView : iPhoneOSGameView
	{
		...
		UIButton upButton, downButton, leftButton, rightButton, lightButton, filterButton, plusButton, minusButton, blendButton;
		...
		private void SetupButtons()
		{
			...
			this.AddButton (ref blendButton, 10, ypos, "Blend");
			ypos-=40;
		...
		{
			using (UIImage img = UIImage.FromFile ("glass.png"))
			{
		...
		private void handleButtons ()
		{
			...
			if (blendButton.TouchInside && !bp)		// Is B Key Pressed And bp FALSE?
			{
				bp=true;							// If So, bp Becomes TRUE
				blend = !blend;						// Toggle blend TRUE / FALSE   
				if(blend)							// Is blend TRUE?
				{
					GL1.Enable (All1.Blend);		// Turn Blending On
					GL1.Disable (All1.DepthTest);	// Turn Depth Testing Off
				}
				else								// Otherwise
				{
					GL1.Disable (All1.Blend);		// Turn Blending Off
					GL1.Enable (All1.DepthTest);	// Turn Depth Testing On
				}
			}
			if (!blendButton.TouchInside)			// Has B Key Been Released?
			{
